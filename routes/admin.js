...
router.get('/restaurant-status', async (req, res) => {
  ...
  try {
    ...
    if (settingsTableExists) {
      ...
      if (hasKeyColumn) {
        console.log('Executing query: SELECT value FROM settings WHERE `key` = ?', ['restaurant_status']);
        ...
        console.warn('Settings table exists but key column not found, using default restaurant status');
      }
    } else {
      console.warn('Settings table not found, using default restaurant status');
    }
...
router.post('/signup', async (req, res) => {
  ...
  if (!name || !email || !password) {
    console.log('Signup failed: Missing fields', { name, email, password });
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    if (!hasOtpColumns) {
      console.log('OTP functionality not supported: Missing otp or otp_expires columns');
      return res.status(500).json({ message: 'OTP functionality not supported in database schema.' });
    }
...
router.post('/logout', async (req, res) => {
  try {
    if (req.session) {
      ...
      console.log('Logout successful: No session found');
      res.status(200).json({ message: 'Logged out successfully.' });
    }
  } catch (error) {
