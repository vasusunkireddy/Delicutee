...
router.get('/admindashboard.html', authenticateToken, ensureAdmin, (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await db.execute(
        ...
        [name, email, phone, hashedPassword, 1, 'admin', 0] // Set role to 'admin'
      );
      ...
      const html = `
        <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: auto; padding: 20px; background-color: #f9f9f9; border-radius: 10px;">
          ...
          <p style="color: #4a5568;">Congratulations! Your Delicute admin account has been successfully created. You're now part of our culinary family, where every bite tells a story.</p>
          <p style="color: #4a5568;">Start managing our delicious offerings today!</p>
      ...
      const sent = await sendEmail(email, 'Delicute - Welcome to Our Admin Team!', html);
      if (!sent) {
    ...
    try {
      ...
      const html = `
        <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: auto; padding: 20px; background-color: #f9f9f9; border-radius: 10px;">
          ...
          <p style="color: #4a5568;">We received a request to reset your admin password. Please use the following OTP to proceed:</p>
          <h3 style="color: #d69e2e; font-size: 24px; text-align: center; margin: 20px 0;">${otp}</h3>
          <p style="color: #4a5568;">This OTP is valid for 10 minutes. Please enter it on the password reset page.</p>
          <p style="color: #4a5568;">If you did not initiate this request, contact us at <a href="mailto:contactdelicute@gmail.com" style="color: #d69e2e;">contactdelicute@gmail.com</a>.</p>
          <p style="color: #4a5568; margin-top: 20px;">Best regards,<br>The Delicute Team</p>
          <p style="color: #718096; font-size: 12px; text-align: center;">© 2025 Delicute. All rights reserved.</p>
      `;
